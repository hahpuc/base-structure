...
export class UserRepository extends Repository<User> {
  ...
  private _applyQueryBase(
    ...
    if (filter) {
      query.andWhere(
        `(user.email LIKE :filter OR user.username LIKE :filter OR profile.full_name LIKE :filter OR profile.zalo_follow_oa_id LIKE :filter
          OR profile.phone LIKE :filter)`,
        {
          filter: `%\\${filter}%`,
        },
      );
